...
def measure_fn(measure: str, input_tensor: Tensor, *args, **kwargs) -> Float[Tensor, '...']:
    ...
    try:
        ...
        raise NotImplementedError(f"Unknown measure function '{measure}'. Available measures:" + ', '.join(
            [f"'{str(fn)}'" for fn in avail_measures.keys()]))
...
class ModelAbliterator:
    def __init__(
        ...
        self.chat_template = chat_template or ChatTemplate(
            self, LLAMA3_CHAT_TEMPLATE)

        self.hidden_size = self.model.cfg.d_model
        self.original_state = {k: v.to('cpu')
        ...
        self.harmful_inst_train, self.harmful_inst_test = prepare_dataset(
            dataset[0])
        self.harmless_inst_train, self.harmless_inst_test = prepare_dataset(
            dataset[1])

        self.fwd_hooks = []
        self.modified = False
        self.activation_layers = [activation_layers] if type(
            activation_layers) == str else activation_layers
    ...
    def save_activations(self, fname: str):
        torch.save([self.harmful, self.harmless, self.modified_layers if self.modified_layers['mlp']
                   or self.modified_layers['W_O'] else None, self.checkpoints if len(self.checkpoints) > 0 else None], fname)
    ...
    def calculate_mean_dirs(self, key: str, include_overall_mean: bool = False) -> Dict[str, Float[Tensor, 'd_model']]:
        ...
        if include_overall_mean:
            if self.harmful[key].shape != self.harmless[key].shape or self.harmful[key].device.type == 'cuda':
                ...
                #
                dirs['mean_dir'] = torch.mean(
                    self.harmful[key] + self.harmless[key], dim=0) / 2.0
    ...
    def refusal_dirs(self, invert: bool = False) -> Dict[str, Float[Tensor, 'd_model']]:
        ...
        refusal_dirs = {key: self.calculate_mean_dirs(
            key) for key in self.harmful if '.0.' not in key}
        if invert:
            refusal_dirs = {key: v['harmless_mean']-v['harmful_mean']
                            for key, v in refusal_dirs.items()}
        else:
            refusal_dirs = {key: v['harmful_mean']-v['harmless_mean']
                            for key, v in refusal_dirs.items()}
    ...
    def layer_attn(self, layer: int, replacement: Float[Tensor, "d_model"] = None) -> Float[Tensor, "d_model"]:
        if replacement is not None and layer not in self._blacklisted:
            ...
            self.modified_layers['W_O'][layer] = self.modified_layers.get(
                layer, [])+[(self.model.blocks[layer].attn.W_O.data.to('cpu'), replacement.to('cpu'))]
    ...
    def layer_mlp(self, layer: int, replacement: Float[Tensor, "d_model"] = None) -> Float[Tensor, "d_model"]:
        if replacement is not None and layer not in self._blacklisted:
            ...
            self.modified_layers['mlp'][layer] = self.modified_layers.get(
                layer, [])+[(self.model.blocks[layer].mlp.W_out.data.to('cpu'), replacement.to('cpu'))]
    ...
    def tokenize_instructions_fn(
        ...
        prompts = [self.chat_template.format(
    ...
    def generate_logits(
        ...
        **kwargs
        ...
        all_toks = torch.zeros(
            (toks.shape[0], toks.shape[1]+max_tokens_generated), dtype=torch.long, device=toks.device)
        ...
        for i in range(max_tokens_generated):
            ...
            if stop_at_eos:
                for batch_idx in generating:
                    generating = [i for i in range(
                        toks.shape[0]) if all_toks[i][-1] != self.model.tokenizer.eos_token_id]
                if len(generating) == 0:
                    break
            ...
            for layer in layers:
                for modifying in [(W_O, self.layer_attn), (mlp, self.layer_mlp)]:
                    if modifying[0]:
                        ...
                        proj = einops.einsum(
                            matrix, refusal_dir.view(-1, 1), '... d_model, d_model single -> ... single') * refusal_dir
        ...
        for layer in layers:
            for modifying in [(W_O, self.layer_attn), (mlp, self.layer_mlp)]:
                if modifying[0]:
                    ...
                    proj = einops.einsum(
                        matrix, refusal_dir.view(-1, 1), '... d_model, d_model single -> ... single') * refusal_dir
                    avg_proj = refusal_dir * \
                        self.get_avg_projections(utils.get_act_name(
                            self.activation_layers[0], layer), refusal_dir)
        ...
        try:
            ...
            if use_hooks:
                ...
                hook_fn = functools.partial(
                    directional_hook, direction=refusal_dir)
                self.fwd_hooks = before_hooks + \
                    [(act_name, hook_fn)
                     for ln, act_name in self.get_all_act_names()]
    ...
    def measure_scores(
        ...
        toks = self.tokenize_instructions_fn(
            instructions=self.harmful_inst_test[:N])
    ...
    def measure_scores_from_logits(
        ...
        max_negative_score_per_sequence = torch.max(
            normalized_negative, dim=-1)[0]
        max_positive_score_per_sequence = torch.max(
            normalized_positive, dim=-1)[0]

        negative_score_per_batch = measure_fn(
            measure, max_negative_score_per_sequence, dim=-1)[0]
        positive_score_per_batch = measure_fn(
            measure, max_positive_score_per_sequence, dim=-1)[0]
    ...
    def create_layer_rankings(
        ...
        harmful_set = torch.isin(
            sorted_harmful_indices, torch.tensor(list(token_set)))
        harmless_set = torch.isin(sorted_harmless_indices, torch.tensor(
            list(token_set if token_set_b is None else token_set_b)))

        indices_in_set = zip(harmful_set.nonzero(as_tuple=True)[
                             1], harmless_set.nonzero(as_tuple=True)[1])
        ...
        for i in tqdm(range(0, min(N, len(toks)), batch_size)):
            ...
            for key in cache:
                if any(k in key for k in self.activation_layers):
                    tensor = torch.mean(
                        cache[key][:, -last_indices:, :], dim=1).to('cpu')
    ...
    def create_activation_cache(
        ...
        for i in tqdm(range(0, min(N, len(toks)), batch_size)):
            logits, cache = self.run_with_cache(toks[i:min(
                i+batch_size, len(toks))], max_new_tokens=measure_refusal, stop_at_layer=stop_at_layer)
            if measure_refusal > 1:
                z_label.extend(self.measure_scores_from_logits(
                    logits, measure_refusal)[0])
            for key in cache:
